...
// Stores values of hull after processing
vector <int> xValue;
vector <int> yValue;
int numPoints;
...
void printHull(iPair a[], int n)
{
    ...
    while (!hull.empty())
    {
        //Attemp to catch values

        int x=(*hull.begin()).first;
        int y=(*hull.begin()).second;
        area=area+x*y;
        xValue.push_back(x);
        yValue.push_back(y);
        cout << "(" << x  << ", "
    ...
    cout<<endl;
    cout<<area/2.0;
    cout<<*xValue.end()<<xValue[2];
}

/*
void polygonArea(xValue,yValue)
{
    numPoints =xValue.size();
    double area = 0;         // Accumulates area in the loop
    int j = numPoints-1;  // The last vertex is the 'previous' one to the first

    for (int i=0; i<numPoints; i++)
    {
        area = area +  (xValue[j]+xValue[i]) * (yValue[j]-yValue[i]);
